...
class Sender:
    def __init__(self, read_length, input_dir, output_dir, n_cpus=8, do_all=False):
        ...
        while len(cycles_todo) > 0:
            ...
            while len(barcodes_todo) > 0:
                ...
                if os.path.exists(inpath):
                    ...
                    if mode == "bam":
                        self.get_unmapped_bam(inpath, outpath, single, do_filter=self.do_filter)
                    elif mode == "fasta":
                        self.get_unmapped_fasta(inpath, outpath, single, do_filter=self.do_filter)
                    else:
